...
parser = argparse.ArgumentParser(description='fmriprep_lifespan_audit entrypoint script.')
...
parser.add_argument('--session_label', help='The label(s) of the session(s) that should be analyzed. The label '
				   ...
				   'participants can be specified with a space separated list.',
				   nargs="+")
parser.add_argument('--subfolder', action='store_true', help='Set this argument to True if your BIDS data and derivative data is stored in a subfolder of an s3 bucket')

# Parse and gather arguments
args = parser.parse_args()
sub_search = args.subfolder
...
# running group level for all subject
	...
	if bids_dir_bucket_name:
		s3_prefix = bids_dir_relative_path
		if sub_search: 
			if len(s3_prefix) > 0:
				if s3_prefix[0] == '/':
	...
	if output_dir_bucket_name:
		s3_prefix = output_dir_relative_path
		if sub_search: 
			if len(s3_prefix) > 0:
				if s3_prefix[0] == '/':
					s3_prefix = s3_prefix[1:]
...
if bids_dir_bucket_name:
	s3_prefix = bids_dir_relative_path + subjects_to_analyze[0]+'/'
	if sub_search:
		if len(s3_prefix) > 0:
			if s3_prefix[0] == '/':
				prefix = s3_prefix[1:]
	...
	s3_prefix = bids_dir_relative_path + 'sub-'
	if sub_search:
		if s3_prefix[0] == '/':
			s3_prefix = s3_prefix[1:]
...
for subject in subjects_to_analyze:
	if bids_dir_bucket_name: # if bids dir is a bucket pull sessions from that subject
		s3_prefix = bids_dir_relative_path + subject+'/'
		if sub_search:
			if len(s3_prefix) > 0:
				if s3_prefix[0] == '/':
					prefix = s3_prefix[1:]
	...
	if sessions_to_analyze:
		for session in sessions_to_analyze:
			...
			if bids_dir_bucket_name:
				s3_prefix = bids_dir_relative_path + subject+ '/' +session
				if sub_search:
					if s3_prefix[0] == '/':
						s3_prefix = s3_prefix[1:]
